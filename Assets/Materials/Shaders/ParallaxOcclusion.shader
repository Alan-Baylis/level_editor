Shader "ParallaxOcclusion" 
{
	...
	SubShader 
	{
		...
		float getHeight(float2 texturePos)
		{
			...
			float4 colorNoise = tex2Dlod(_NoiseTex, float4(texturePos * 1, 0, 0));
		...
		{						
			...
			//Water
			if (colorGrayscale < 0.0)
			{
				mixedColor = 1;//tex2Dlod(_NoiseTex, float4(texturePos * 0.2, 0, 0)) * _WaterColor;
			}
