...
namespace RealtimeCSG.Foundation
{
	partial struct CSGTreeBrush
	{
		...
		[DllImport(CSGManager.NativePluginName, CallingConvention = CallingConvention.Cdecl)] private static extern bool  GetBrushToTreeSpace(Int32 brushNodeID, [Out] out Matrix4x4 brushToTreeSpace);
		...
		private static Matrix4x4 GetBrushToTreeSpace(Int32 brushNodeID)
		{
			Matrix4x4 result = Matrix4x4.identity;
			if (GetBrushToTreeSpace(brushNodeID, out result))
				return result;
			return Matrix4x4.identity;
		}

		[DllImport(CSGManager.NativePluginName, CallingConvention = CallingConvention.Cdecl)] private static extern bool SetBrushToTreeSpace(Int32 brushNodeID, [In] ref Matrix4x4 brushToTreeSpace);
