...
namespace RealtimeCSG.Legacy
{
	...
	partial class BrushFactory
	{
		...
		public static SurfaceLayers CreateSurfaceLayer(TexGen texGen, TexGenFlags texGenFlags)
		{
			...
			layers.layerUsage		= (((texGenFlags & TexGenFlags.NoRender        ) == 0) ? LayerUsageFlags.Renderable     : LayerUsageFlags.None) |
									  (((texGenFlags & TexGenFlags.NoCastShadows   ) == 0) ? LayerUsageFlags.CastShadows    : LayerUsageFlags.None) |
									  (((texGenFlags & TexGenFlags.NoReceiveShadows) == 0) ? LayerUsageFlags.ReceiveShadows : LayerUsageFlags.None) |
									  (((texGenFlags & TexGenFlags.NoCollision     ) == 0) ? LayerUsageFlags.Collidable       : LayerUsageFlags.None);
