	...
	internal partial class InternalCSGModelManager
	{
		...
		public static void EnsureInitialized(CSGBrush component)
		{
			...
			if (statement4)
			{
				...
				brushCache.hierarchyItem.NodeID			= component.brushNodeID;
			}
		...
		public static void OnPassthroughChanged(CSGOperation component)
		{
			if (component.PassThrough)
			{
				...
				if (operationCache == null)
					return;
				var parent = operationCache.ChildData.Parent;
				if (parent)
				{
					CreateOperationCache(parent);
				} else
				{
					var model = operationCache.ChildData.Model;
					CreateModelCache(model);
				}
			}
			CreateOperationCache(component);
		}
		...
		public static void EnsureInitialized(CSGOperation component)
		{
				...
				component.operationNodeID != CSGNode.InvalidNodeID)
			{
				...
				operationCache.ParentData.NodeID		= component.operationNodeID;
			}
		}
		...
		public static void OnTransformChildrenChanged(CSGModel component)
		{
			...
			MeshInstanceManager.ValidateModel(component, modelCache, checkChildren: true);
		}
		...
		public static void EnsureInitialized(CSGModel component)
		{
				...
				component.modelNodeID != CSGNode.InvalidNodeID)
			{
				...
				modelCache.ParentData.NodeID		= component.modelNodeID;
			}
		}
		...
		static void RegisterBrush(CSGBrush brush)
		{
			...
			InitializeHierarchy(brushCache.ChildData, brushTransform);
			InitializeHierarchy(brushCache, brushTransform);
			...
			{
				var brushToModelSpace = brushCache.compareTransformation.brushToModelSpaceMatrix;
		...
		static void RegisterOperation(CSGOperation op)
		{
			...
			var operationCache = InternalCSGModelManager.GetOperationCache(op, forceRecreate: true);

			EnsureInitialized(op);
			
			InitializeHierarchy(operationCache.ChildData, op.transform);
			operationCache.PrevOperation	= op.OperationType;
			operationCache.PrevPassThrough	= op.PassThrough;
			...
			if (!operationCache.ChildData.Model)
			{
				...
				operationCache.ParentData.NodeID = CSGNode.InvalidNodeID;
			...
			Operations.Add(op);
			operationCache.ParentData.NodeID = opID;
			_isHierarchyModified = true;
		}
		#endregion 
		...
		private static void RegisterModel(CSGModel model)
		{
			...
			External.SetModelEnabled(model.modelNodeID, modelCache.IsEnabled);
		...
		internal static void UpdateRegistration()
		{
			...
			if (RegisterModels.Count > 0 || EnableModels.Count > 0 || RegisterOperations.Count > 0 || RegisterBrushes.Count > 0)
			{
				...
				for (var i = 0; i < registerModelsList.Length; i++)
				{
					...
					if (modelCache == null)
						continue;
					MeshInstanceManager.ValidateModel(component, modelCache);
				}
			}
