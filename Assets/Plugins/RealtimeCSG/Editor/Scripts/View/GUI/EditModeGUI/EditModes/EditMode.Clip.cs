...
//		[NonSerialized] bool			onHandle		= false;
		...
		int RememberTargetData(CSGBrush[] addedBrushes = null)
		{
			...
			for (int srcIndex = srcBrushes.Length - 1; srcIndex >= 0; srcIndex--)
			{
				...
				backupData[dstIndex].localToWorldMatrix	= brush_cache.compareTransformation.localToWorldMatrix;
		...
		void CreateFirstPoint()
		{
			...
			if (currentOnBrush != null)
			{
					...
					brush_cache.ChildData.ModelTransform != null)
				{
					...
					if (SceneQueryUtility.FindBrushIntersection(currentOnBrush, brush_cache.ChildData.ModelTransform.localToWorldMatrix, ray_start, ray_end, out intersectionFar))
					{
