...
namespace RealtimeCSG
{
	internal sealed class EditModeSurface : ScriptableObject, IEditMode
	{
		...
		bool UpdateGrid()
		{
			...
			var zero_point		= SurfaceUtility.ConvertTextureToWorldCoord(brush, hoverOnSurfaceIndex, MathConstants.zeroVector2);
		...
		void DrawOutlines()
		{
				...
				prevHoverBrushNodeID   != hoverOnBrushNodeID)
			{
				...
				for (int t = 0; t < this.brushTargets.Length; t++)
				{
					...
					var brush_translation	= brush_cache.compareTransformation.localToWorldMatrix;
					var brush_shape			= brush.Shape;
				...
				switch (Event.current.type)
				{
					...
					case EventType.MouseDrag:
					{
						...
						if (Tools.current == Tool.Move)
						{
							var brush				= brushTargets[hoverOnTarget];							
							InternalCSGModelManager.SetBrushMeshSurfaces(brush);
							snappingOffset			= SurfaceUtility.ConvertTextureToWorldCoord(brush, hoverOnSurfaceIndex, MathConstants.zeroVector2);
						}
